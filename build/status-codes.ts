...
generate({
	...
	generateFn: (destPath, concept): void => {
		...
		for (const conceptValue of concept.values) {
			...
			const statusCodeTypeName = `HttpStatusCode${
				getMethodTypeName(statusCodeName)
			}`
			...
			switch (digit) {
				case '1':
					categorized['1'].push(statusCodeName)
					break
				case '2':
					categorized['2'].push(statusCodeName)
					break
				case '3':
					categorized['3'].push(statusCodeName)
					break
				case '4':
					categorized['4'].push(statusCodeName)
					break
				case '5':
					categorized['5'].push(statusCodeName)
					break
			}
		}
